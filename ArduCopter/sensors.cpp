...
void Copter::read_rangefinder(void)
{
    ...
    rangefinder_state.alt_healthy = ((rangefinder.status(ROTATION_PITCH_270) == RangeFinder::RangeFinder_Good) && (rangefinder.range_valid_count(ROTATION_PITCH_270) >= RANGEFINDER_HEALTH_MAX));

    int16_t temp_alt = rangefinder.distance_cm(ROTATION_PITCH_270);
...
void Copter::update_sensor_status_flags(void)
{
    ...
    if (rangefinder_state.enabled) {
        ...
        if (rangefinder.has_data(ROTATION_PITCH_270)) {
            control_sensors_health |= MAV_SYS_STATUS_SENSOR_LASER_POSITION;
        }
    }
#endif
