...
void Copter::check_dynamic_flight(void)
{
    ...
    if (!moving && rangefinder_state.enabled && rangefinder.status(ROTATION_PITCH_270) == RangeFinder::RangeFinder_Good) {
        // when we are more than 2m from the ground with good
        // rangefinder lock consider it to be dynamic flight
        moving = (rangefinder.distance_cm(ROTATION_PITCH_270) > 200);
    }
